...
#Incidents by country pairs by years dataset Reading
incidents_count_by_country_pairs_years = db.APT_Total_incidents_by_country_pairs_years
incidents_count_country_pairs_years = incidents_count_by_country_pairs_years.find_one()
...
def geo_data(doc):
    ...
    if len(doc["cameo_operation_countries"]) < 30:
        ...
        for second_country_cameo in doc["cameo_operation_countries"]:
            ...
            if first_country_cameo != second_country_cameo:
                ...
                for year in range(bound_year, int(op_year)+1):
                    ...
                    if year < 2015:
                        #GDELT 1.0
                        #Recycling previous calculations
                        rec_flag = True
                        for rec_doc in geo_set.find():
                            #We can recycle
                            if rec_doc["country1_code"] == first_country_cameo and rec_doc["country2_code"] == second_country_cameo and bound_year <= int(rec_doc["incident_year"]):
                                for rec_year in range((int(rec_doc["incident_year"])-5), (int(rec_doc["incident_year"])+1)):
                                    if rec_year == year:
                                        
                                        #Recycling values
                                        rec_tag = str(int(rec_doc["incident_year"])-rec_year)
                                        
                                        pos_golds_numb = doc["positive_goldstein_mentions_"+rec_tag]
                                        neg_golds_numb = doc["negative_goldstein_mentions_"+rec_tag]
                                        year_means.append(doc["golds_mean_"+rec_tag])
                                        year_stds.append(doc["golds_std_"+rec_tag])
                                        rec_flag = False
                        ...
                        #GDELT 2.0
                        gdelt_data = gcp2df("SELECT GoldsteinScale FROM `gdelt-bq.full.events` WHERE Actor1Code LIKE '"+first_country_cameo+"' AND Actor2Code LIKE '"+second_country_cameo+"' AND Year = "+str(year)+" LIMIT 25000")
                                    
                        if len(gdelt_data) > 0:   
                            #Number of positive goldstein indexes
                            pos_golds = len(gdelt_data[(gdelt_data["GoldsteinScale"] >= 0)])
                            
                            #Add number of positive goldstein indexes
                            pos_golds_numb = pos_golds/len(gdelt_data)
                            
                            #Ass number of negative goldstein indexes
                            neg_golds_numb = (len(gdelt_data) - pos_golds)/len(gdelt_data)
                            
                            
                            #Calular media Goldstein
                            #Append to list
                            year_means.append(np.mean(gdelt_data["GoldsteinScale"].to_numpy()))
                        
                            #Calcular desviación típica
                            #Append to dict
                            year_stds.append(np.std(gdelt_data["GoldsteinScale"].to_numpy()))
                    ...
                    try:
                        ...
                        if pos_golds_numb > 0:
                            ...
                            #Store number of incidents current year
                            try:
                                num_incidents = incidents_count_country_pairs_years[str(year)+"_"+first_country_cameo+"_"+second_country_cameo]
                            except:
                                num_incidents = 0
                                
                            dataset_row['incidents_'+tag] = num_incidents
                            
                            if year < 2015:
                                
                                dataset_row["positive_goldstein_mentions_"+tag] = pos_golds_numb/(pos_golds_numb+neg_golds_numb)
                                dataset_row["negative_goldstein_mentions_"+tag] = neg_golds_numb/(pos_golds_numb+neg_golds_numb)
                            ...
                            dataset_row["golds_mean_"+tag] = np.mean(numpy_year_means)
                            dataset_row["golds_std_"+tag] = np.mean(numpy_year_stds)
...
first_country_cameo = 'RUS'
second_country_cameo = 'BEL'
year = 2017


gdelt_data = gcp2df("SELECT GoldsteinScale FROM `gdelt-bq.full.events` WHERE Actor1Code LIKE '"+first_country_cameo+"' AND Actor2Code LIKE '"+second_country_cameo+"' AND Year = "+str(year)+" LIMIT 5000")


print(np.mean(gdelt_data["GoldsteinScale"].to_numpy(), dtype=np.float64)))                             
...
#print (df["GoldsteinScale"].to_numpy())
"""